...
// Ommit the 'authorization' headers to use on the endpoints except the 'create'
const headers = headersCreate.omit({ authorization: true });

// Object endpoints schemas --------------------------------------------
export const endpoints = {
  create: {
    method: 'POST',
    hasHeaders: headersCreate,
    hasBody: z.object({
  ...
  mbway: {
    ...
    hasBody: z
      .object({
        transactionID: z.string().length(20, { message: 'TransactionID must be exactly 20 characters long' }),
        transactionSignature: z.string().nonempty({ message: 'TransactionSignature is required' }),
        customerPhone: z
          .string()
